    ...
    const respond = async (response: Notifications.NotificationResponse) => {
      ...
      try {
        if (response.actionIdentifier === 'done' && ref && spaceId) {
          ...
          const [snapshot, spaceSnapshot] = await Promise.all([
            getDoc(ref),
            getDoc(doc(db, 'spaces', spaceId)),
          ]);
          const reminder = snapshot.data();
          const memberIds = (spaceSnapshot.data()?.memberIds as string[] | undefined) ?? [];
          ...
          if (repeat && dueAt) {
            ...
            // The screen removes one-off reminders after its undo window. There is no undo
            // surface in the notification shade, so the direct action can remove it now
            // instead of accumulating invisible `done` documents forever.
            await deleteDoc(ref);
          }
