/**
 * The exact legal text, verbatim — this file only stores it. Rendering (headings, bold,
 * links, lists) is handled by the markdown parser in @/lib/markdown, so nothing here should
 * be summarized, reworded, or reformatted relative to the source document.
 */
export const TermsVersion = '2026-07-28';

export const TermsMarkdown = `# Términos de uso de Flare

**Última actualización: 28 de julio de 2026**

Estos Términos de uso regulan el acceso, registro y utilización de la aplicación **Flare** y de los servicios asociados a ella.

Lee estas condiciones antes de crear una cuenta o utilizar la aplicación. Al registrarte, marcar la casilla de aceptación correspondiente o continuar utilizando Flare, confirmas que has leído y aceptas estos Términos de uso y nuestra Política de Privacidad.

## 1. Qué es Flare

Flare es una aplicación que permite crear espacios digitales privados: un espacio personal, de un solo miembro, y espacios compartidos de hasta ocho personas.

Dentro de esos espacios, las personas usuarias pueden compartir contenidos y utilizar las funcionalidades disponibles en cada momento, como fotografías, mensajes, avisos, recordatorios u otros elementos similares.

Flare proporciona la infraestructura tecnológica necesaria para que las personas de cada espacio puedan utilizarlo. No somos autores ni propietarios del contenido que suben las personas usuarias y no intervenimos en sus comunicaciones, salvo cuando sea necesario por razones técnicas, de seguridad, para atender un reporte o para cumplir una obligación legal.

## 2. Requisitos para utilizar Flare

Para crear una cuenta y utilizar Flare debes:

- Tener al menos **16 años**.
- Tener capacidad legal suficiente para aceptar estos Términos.
- Proporcionar información verdadera y actualizada.
- Utilizar la aplicación únicamente para fines personales y lícitos.
- Cumplir estos Términos y la legislación aplicable.

Al registrarte, declaras que cumples estos requisitos.

Flare no está dirigida a menores de edad. Si detectamos que una persona menor de 16 años ha creado una cuenta, podremos suspenderla o eliminarla y borrar sus datos conforme a nuestra Política de Privacidad.

## 3. Registro y seguridad de la cuenta

Eres responsable de mantener la confidencialidad de tus credenciales de acceso y de las acciones realizadas desde tu cuenta.

No debes:

- Compartir tus credenciales con otras personas.
- Permitir que otra persona utilice tu cuenta.
- Acceder o intentar acceder a la cuenta de otra persona sin autorización.
- Crear una cuenta utilizando la identidad o los datos de otra persona.
- Eludir las medidas de autenticación o seguridad de la aplicación.

Debes avisarnos cuanto antes a [info@wearecapa.es](mailto:info@wearecapa.es) si sospechas que alguien ha accedido a tu cuenta sin permiso o que existe cualquier incidente de seguridad relacionado con ella.

Podremos pedirte información razonablemente necesaria para comprobar tu identidad o recuperar el acceso a la cuenta.

## 4. Creación de espacios

Cada cuenta dispone de un espacio personal, de un solo miembro, desde el momento de su registro.

Además, las personas usuarias pueden crear espacios compartidos de hasta **ocho personas** — pareja, familia, un piso, un viaje u otro grupo similar — o unirse a uno ya existente.

La incorporación a un espacio compartido se realiza mediante una llave de acceso, enlace, código u otro mecanismo habilitado en la aplicación. Quien recibe la llave decide libremente si se une al espacio.

Eres responsable de compartir la llave de acceso únicamente con las personas con las que deseas compartir ese espacio.

Cada persona conserva el control de su propia cuenta y puede abandonar un espacio compartido conforme a las opciones disponibles en la aplicación. Si permanecen otras personas en él, el espacio continúa existiendo para ellas.

La creación o el uso de un espacio compartido no implica que Flare verifique la identidad, relación personal, consentimiento general o situación legal existente entre las personas que lo forman.

## 5. Responsabilidad sobre el contenido

Cada persona es responsable del contenido que sube, publica, envía o comparte mediante Flare.

Antes de subir cualquier contenido, debes asegurarte de que:

- Eres su autor o titular, o tienes permiso suficiente para utilizarlo.
- Su publicación y comunicación a las demás personas del espacio es lícita.
- Cuentas con el consentimiento necesario de las personas que aparecen o son identificables.
- No vulnera la privacidad, intimidad, honor, imagen, propiedad intelectual u otros derechos de terceros.
- No contiene información obtenida de forma ilícita.
- No incumple estos Términos ni la legislación aplicable.

El hecho de que un espacio sea privado no convierte en lícito un contenido que sería ilegal o vulneraría derechos fuera de la aplicación.

## 6. Contenido y conductas prohibidas

No puedes utilizar Flare para crear, subir, almacenar, enviar, solicitar, compartir o facilitar:

- Contenido ilegal o que promueva actividades ilegales.
- Material de abuso o explotación sexual infantil, o cualquier contenido sexual en el que aparezcan o puedan aparecer menores de edad.
- Imágenes íntimas o sexuales de otra persona compartidas sin su consentimiento.
- Contenido obtenido mediante coacción, engaño, acceso no autorizado o vulneración de la intimidad.
- Amenazas, acoso, hostigamiento, chantaje, extorsión o intimidación.
- Contenido que fomente la violencia o suponga una amenaza creíble para una persona.
- Contenido difamatorio o que vulnere el honor, la privacidad o la propia imagen.
- Contenido que infrinja derechos de autor, marcas u otros derechos de propiedad intelectual.
- Datos personales de terceros obtenidos o difundidos sin una base legítima.
- Contenido discriminatorio, de odio o que incite a la violencia contra personas o colectivos.
- Malware, virus, código malicioso o mecanismos destinados a dañar, interferir o acceder sin autorización a sistemas o dispositivos.
- Contenido utilizado para suplantar la identidad de otra persona.
- Cualquier otro contenido contrario a estos Términos o a la legislación aplicable.

Tampoco puedes:

- Utilizar Flare para vigilar, controlar o localizar a otra persona sin su consentimiento.
- Presionar a otra persona para que comparta contenido íntimo o personal.
- Descargar, copiar o difundir fuera de Flare contenido de la otra persona sin su permiso, salvo que exista una base legal para hacerlo.
- Utilizar sistemas automatizados para extraer información de la aplicación.
- Interferir en el funcionamiento o la seguridad del servicio.
- Intentar acceder a áreas, cuentas, datos o funcionalidades para las que no tienes autorización.
- Utilizar la aplicación con fines comerciales, fraudulentos o distintos de su finalidad personal sin nuestra autorización previa.

## 7. Propiedad del contenido

Conservas los derechos que te correspondan sobre el contenido que subes a Flare.

Flare no adquiere la propiedad de tus fotografías, mensajes ni demás contenidos.

Al subir contenido, nos concedes una licencia limitada, no exclusiva, gratuita y válida durante el tiempo necesario para:

- Almacenar y alojar el contenido.
- Procesarlo técnicamente.
- Mostrarlo dentro de cada espacio a las personas autorizadas.
- Sincronizarlo entre dispositivos.
- Crear copias de seguridad técnicas.
- Mantener la seguridad y el funcionamiento del servicio.
- Atender reportes, incidencias o requerimientos legalmente válidos.

Esta licencia se limita exclusivamente a la prestación, mantenimiento y protección de Flare. No utilizaremos tu contenido para publicidad ni lo venderemos a terceros.

La licencia termina cuando el contenido se elimina definitivamente de nuestros sistemas, salvo que sea necesario conservarlo temporalmente por motivos técnicos, para cumplir una obligación legal, atender una reclamación o proteger derechos y seguridad.

## 8. Propiedad de Flare

La aplicación, su diseño, código, marca, nombre, logotipos, interfaces, textos propios, estructura y demás elementos desarrollados para Flare pertenecen a We Are Capa o se utilizan con autorización de sus respectivos titulares.

La aceptación de estos Términos te concede un derecho personal, limitado, revocable, no exclusivo, intransferible y no sublicenciable para utilizar la aplicación conforme a su finalidad.

No puedes copiar, modificar, distribuir, vender, alquilar, descompilar, realizar ingeniería inversa o explotar Flare o cualquiera de sus elementos, salvo en los casos expresamente permitidos por la legislación aplicable.

## 9. Privacidad del espacio y límites de confidencialidad

Cada espacio compartido está diseñado para que su contenido sea accesible únicamente por las cuentas que forman parte de él, salvo los accesos técnicos, de seguridad o legalmente necesarios que se describen en estos Términos y en la Política de Privacidad.

No obstante, debes tener en cuenta que cualquier otra persona del espacio puede:

- Ver el contenido que compartes.
- Realizar capturas de pantalla.
- Descargarlo, copiarlo o conservarlo fuera de Flare.
- Mostrarlo a terceros utilizando medios externos a la aplicación.

Flare no puede impedir ni revertir las copias o capturas realizadas fuera de sus sistemas. Por ello, solo debes compartir contenido con el que te sientas seguro y respecto del cual confíes en las demás personas del espacio.

La consideración de un espacio como privado no supone una garantía absoluta de confidencialidad frente a acciones realizadas por otras personas del espacio, ataques informáticos, fallos técnicos o requerimientos válidos de las autoridades.

## 10. Proveedores tecnológicos

Para prestar el servicio podemos utilizar proveedores tecnológicos que realizan funciones como alojamiento, almacenamiento de archivos, bases de datos, autenticación, notificaciones, seguridad, análisis técnico o mantenimiento.

Entre estos proveedores pueden encontrarse **Google Firebase**, **Cloudinary** y **GIPHY**, además de otros que se identifiquen en la Política de Privacidad.

Estos proveedores podrán procesar datos y contenido únicamente en la medida necesaria para prestar sus servicios, cumplir nuestras instrucciones, proteger sus sistemas o cumplir la legislación aplicable.

No afirmamos que todo el contenido sea revisado o escaneado automáticamente. Podremos implementar herramientas técnicas de seguridad o moderación cuando sean necesarias, proporcionales, estén legalmente permitidas y se informe de ellas conforme a la normativa aplicable.

## 11. Moderación y conocimiento de contenidos ilegales

Flare no revisa previamente todo el contenido que las personas usuarias comparten y no garantiza que vaya a detectar de manera automática todos los usos indebidos.

Cuando tengamos conocimiento efectivo de un contenido presuntamente ilegal o contrario a estos Términos, podremos revisarlo y adoptar las medidas necesarias.

Dependiendo de las circunstancias, estas medidas pueden incluir:

- Limitar temporalmente el acceso al contenido.
- Retirar o bloquear el contenido.
- Advertir a la persona responsable.
- Restringir determinadas funciones.
- Desvincular cuentas.
- Suspender temporalmente una cuenta.
- Cerrar definitivamente una cuenta.
- Conservar determinada información cuando sea legalmente necesario.
- Informar a las autoridades competentes cuando exista una obligación legal o un riesgo grave para una persona.

Siempre que sea posible y legalmente adecuado, informaremos a la persona afectada de la medida adoptada y de sus motivos. Podremos limitar la información comunicada cuando sea necesario para proteger a terceros, preservar una investigación, prevenir nuevos daños o cumplir una obligación legal.

## 12. Cómo reportar contenido o conductas

Puedes reportar contenido o conductas que consideres ilegales, contrarias a estos Términos o lesivas para tus derechos escribiendo a:

[info@wearecapa.es](mailto:info@wearecapa.es)

Para que podamos valorar el reporte, incluye, cuando sea posible:

- Tu nombre y un medio de contacto.
- Una descripción clara del contenido o conducta.
- La cuenta o espacio afectado.
- La ubicación del contenido dentro de la aplicación.
- El motivo por el que consideras que es ilegal o contrario a estos Términos.
- Cualquier información o prueba que ayude a localizarlo y evaluarlo.
- Una declaración de que la información aportada es correcta y se facilita de buena fe.

Confirmaremos la recepción del reporte y lo revisaremos sin demora indebida. Como plazo ordinario, intentaremos responder en un máximo de **10 días hábiles**, aunque los casos complejos pueden requerir más tiempo.

Los reportes relacionados con riesgo para la vida o integridad de una persona, menores de edad, explotación sexual, amenazas graves, extorsión o difusión no consentida de imágenes íntimas serán tratados con carácter prioritario.

Si existe un peligro inmediato, debes contactar directamente con los servicios de emergencia o las Fuerzas y Cuerpos de Seguridad correspondientes.

El envío deliberado de reportes falsos, abusivos o destinados a perjudicar a otra persona puede dar lugar a restricciones en la cuenta.

## 13. Colaboración con las autoridades

Cuando exista una obligación legal, una orden válida o una situación prevista por la normativa aplicable, podremos conservar y comunicar a las autoridades competentes la información de la que dispongamos.

Esto puede incluir información de cuenta, datos técnicos, registros de acceso, contenido relacionado con el requerimiento u otros datos necesarios y proporcionados al objeto de la solicitud.

También podremos informar de hechos que puedan constituir delitos graves o que impliquen un riesgo para la vida, la integridad o la seguridad de una persona cuando la legislación lo permita o lo exija.

No comunicaremos datos a una autoridad basándonos únicamente en solicitudes informales que no tengan una base jurídica válida.

## 14. Borrado de contenido

Puedes borrar el contenido que hayas subido utilizando las opciones disponibles en la aplicación.

Cuando eliminas un contenido:

- Deja de estar disponible para el uso ordinario dentro del espacio compartido.
- Las demás personas del espacio dejan de poder acceder a él a través de Flare, salvo que el contenido forme parte de una función o registro que legítimamente deba conservarse.
- La eliminación no afecta a capturas de pantalla, descargas, copias o reenvíos realizados previamente fuera de la aplicación.
- Pueden permanecer copias residuales durante un periodo limitado en sistemas de respaldo o recuperación.
- Podremos conservarlo cuando sea necesario para cumplir una obligación legal, atender una reclamación, investigar un uso indebido o proteger derechos y seguridad.

Los plazos y criterios de conservación se explican con mayor detalle en la Política de Privacidad.

## 15. Salida de un espacio compartido

Cualquier persona puede abandonar un espacio compartido mediante las funciones disponibles en Flare.

Cuando alguien abandona un espacio en el que permanecen otras personas, el espacio y su contenido continúan existiendo para quienes se quedan, y quien lo abandona pierde el acceso a él.

Cuando alguien abandona un espacio en el que no queda ninguna otra persona, el espacio se elimina junto con su contenido, conforme se explica en la Política de Privacidad.

Al abandonar un espacio:

- Pierdes el acceso al contenido que hubiera en él, incluido el que tú mismo hayas subido.
- Las demás personas del espacio pueden conservar el contenido que permanezca en él, cuando técnicamente sea posible.
- Las copias realizadas previamente fuera de la aplicación no se eliminan automáticamente.

Si existe una disputa entre personas de un mismo espacio sobre la propiedad, acceso o eliminación de un contenido, podremos bloquearlo temporalmente mientras revisamos la situación, pero no actuamos como mediadores de la relación personal entre ellas ni resolvemos controversias privadas sobre hechos que no podamos verificar.

## 16. Eliminación de la cuenta

Puedes solicitar la eliminación de tu cuenta desde la aplicación, cuando esta función esté disponible, o escribiendo a [info@wearecapa.es](mailto:info@wearecapa.es).

La eliminación de la cuenta puede conllevar:

- La pérdida de acceso a Flare.
- La eliminación de tu espacio personal y de su contenido.
- Tu salida de cada espacio compartido del que formes parte: continuará existiendo para las demás personas si permanece alguna en él, o se eliminará junto con su contenido si eras la única persona que quedaba.
- La eliminación o anonimización de tus datos personales.
- La conservación temporal de ciertos datos o copias de seguridad cuando resulte necesario por razones técnicas o legales.

Antes de eliminar tu cuenta, podrás descargar o guardar el contenido que desees conservar cuando Flare ofrezca una función para ello. No garantizamos que exista una herramienta de exportación para todos los tipos de contenido.

La eliminación de la aplicación de tu dispositivo no elimina automáticamente tu cuenta.

## 17. Suspensión o cierre por parte de Flare

Podremos restringir, suspender o cerrar una cuenta cuando:

- Se incumplan estos Términos.
- Exista un riesgo para otras personas o para la seguridad del servicio.
- Detectemos fraude, suplantación o accesos no autorizados.
- Sea necesario cumplir una obligación legal o una orden de una autoridad.
- La cuenta permanezca inactiva durante un periodo prolongado y se haya informado previamente de esta posibilidad.
- Se produzca un uso que pueda generar responsabilidad o perjuicios relevantes para Flare o terceros.
- Dejemos de prestar el servicio.

Salvo en casos urgentes, graves o legalmente restringidos, intentaremos informar a la persona afectada de la medida y de sus motivos.

Podrás solicitar la revisión de una decisión escribiendo a [info@wearecapa.es](mailto:info@wearecapa.es) y aportando la información que consideres relevante.

## 18. Disponibilidad y cambios en el servicio

Trabajamos para mantener Flare disponible y en buen funcionamiento, pero no garantizamos que el servicio sea permanente, ininterrumpido o completamente libre de errores.

Podemos realizar tareas de mantenimiento, actualizaciones, modificaciones técnicas o cambios de funcionalidad. Cuando sea razonablemente posible, informaremos previamente de las interrupciones o cambios que afecten de manera relevante al servicio.

También podremos añadir, modificar o retirar funcionalidades por razones técnicas, de seguridad, legales, operativas o comerciales.

Si un cambio reduce de manera sustancial una funcionalidad esencial, intentaremos comunicarlo con una antelación razonable y ofrecer, cuando resulte posible, opciones para descargar o eliminar el contenido afectado.

## 19. Seguridad y copias de respaldo

Aplicamos medidas técnicas y organizativas razonables para proteger el servicio y los datos tratados.

Sin embargo, ningún sistema informático es completamente seguro. No podemos garantizar que nunca se produzcan accesos no autorizados, errores, pérdida accidental, ataques, vulnerabilidades o fallos de proveedores externos.

Podremos realizar copias de seguridad con fines de continuidad y recuperación del servicio. Estas copias no estarán destinadas al acceso ordinario de las personas usuarias y se eliminarán o sobrescribirán conforme a nuestros plazos internos de conservación.

Eres responsable de conservar por otros medios cualquier contenido que consideres imprescindible. Flare no debe utilizarse como único sistema de archivo o copia permanente de fotografías, mensajes o documentos importantes.

## 20. Servicios y enlaces de terceros

La aplicación puede integrar servicios, bibliotecas, enlaces o funcionalidades proporcionadas por terceros.

El uso de esos servicios puede estar sujeto a sus propias condiciones y políticas de privacidad. Flare no controla ni responde de servicios externos que no gestione directamente, sin perjuicio de las responsabilidades que legalmente nos correspondan por la elección y utilización de nuestros proveedores.

Si descargas Flare desde una tienda de aplicaciones, también pueden resultar aplicables las condiciones de dicha tienda. En caso de contradicción, estos Términos regularán tu relación con Flare en la medida legalmente permitida.

## 21. Funciones gratuitas y posibles servicios de pago

Flare podrá ofrecer funciones gratuitas y, en el futuro, incorporar funciones, planes o suscripciones de pago.

Antes de contratar cualquier servicio de pago, se informará de manera clara sobre:

- El precio total y los impuestos aplicables.
- Las funcionalidades incluidas.
- La duración.
- La forma de pago.
- Las condiciones de renovación y cancelación.
- El derecho de desistimiento, cuando resulte aplicable.
- Cualquier otra condición relevante.

La incorporación de servicios de pago no modificará retroactivamente las condiciones económicas de un servicio ya contratado sin una base válida y sin la información previa exigida por la legislación aplicable.

## 22. Responsabilidad

Flare se presta conforme a la funcionalidad disponible en cada momento.

En la máxima medida permitida por la legislación aplicable, no somos responsables de:

- Conflictos personales entre las personas de un mismo espacio.
- La veracidad, legalidad o consecuencias del contenido que suben las personas usuarias.
- Capturas, descargas, copias o divulgaciones realizadas por otra persona fuera de Flare.
- Daños causados por el uso ilícito o no autorizado de una cuenta por parte de terceros cuando no sea imputable a Flare.
- Interrupciones, retrasos o fallos causados por circunstancias fuera de nuestro control razonable.
- Pérdidas derivadas de utilizar Flare como único sistema de almacenamiento.
- Servicios, dispositivos, redes o aplicaciones de terceros que no controlemos.

Nada de lo dispuesto en estos Términos excluye ni limita:

- Los derechos irrenunciables de las personas consumidoras.
- Nuestra responsabilidad por dolo, culpa o negligencia cuando legalmente corresponda.
- La responsabilidad por daños personales o por cualquier otra materia que no pueda limitarse legalmente.
- Las obligaciones que nos correspondan en materia de protección de datos, seguridad o servicios digitales.

## 23. Indemnidad por usos ilícitos

Cuando la legislación lo permita, serás responsable frente a Flare de los daños, costes o reclamaciones que se deriven directamente de un uso doloso, fraudulento o manifiestamente ilícito de la aplicación o de una vulneración grave de derechos de terceros imputable a tu actuación.

Esta cláusula no limita tus derechos como consumidor ni te obliga a asumir riesgos que legalmente correspondan a Flare.

## 24. Política de Privacidad

El tratamiento de datos personales se regula en nuestra **Política de Privacidad**, que forma parte de la información legal de Flare.

La Política de Privacidad explica, entre otros aspectos:

- Qué datos tratamos.
- Para qué los utilizamos.
- Las bases jurídicas del tratamiento.
- Los proveedores que intervienen.
- Los plazos de conservación.
- Las posibles transferencias internacionales.
- Los derechos de las personas usuarias.
- Cómo contactar con nosotros o reclamar ante la autoridad de control.

Debes leer la Política de Privacidad antes de utilizar la aplicación.

## 25. Cambios en estos Términos

Podemos modificar estos Términos cuando exista un motivo válido, como:

- Cambios legales o regulatorios.
- Modificaciones técnicas o de seguridad.
- Incorporación, modificación o retirada de funcionalidades.
- Cambios en el modelo de prestación del servicio.
- Necesidad de prevenir abusos, fraudes o riesgos.
- Corrección de errores o mejora de la claridad del documento.

Cuando los cambios afecten de forma sustancial a tus derechos u obligaciones, te informaremos con una antelación razonable mediante la aplicación, correo electrónico u otro medio adecuado.

La comunicación indicará la fecha de entrada en vigor. Si no estás de acuerdo con los cambios, podrás dejar de utilizar Flare y eliminar tu cuenta antes de esa fecha.

Los cambios que resulten necesarios para cumplir inmediatamente una obligación legal, solucionar una vulnerabilidad grave o impedir un daño podrán aplicarse sin preaviso, informándote tan pronto como resulte razonablemente posible.

## 26. Finalización del servicio

Podremos dejar de ofrecer Flare total o parcialmente por motivos técnicos, legales, económicos u operativos.

Cuando sea posible, avisaremos con una antelación razonable y explicaremos qué ocurrirá con las cuentas y el contenido.

Si el cierre no responde a una situación urgente, intentaremos proporcionar un periodo para que las personas usuarias puedan revisar, descargar o eliminar su contenido cuando exista una herramienta técnica disponible para hacerlo.

## 27. Nulidad parcial

Si una cláusula de estos Términos fuera declarada nula, inválida o inaplicable, esa circunstancia no afectará al resto del documento.

La cláusula afectada se interpretará o sustituirá, en la medida legalmente posible, por otra válida que se aproxime a su finalidad original y respete los derechos de las personas usuarias.

## 28. No renuncia

El hecho de que Flare no ejerza de inmediato un derecho o no actúe ante un incumplimiento concreto no implica que renuncie a ejercerlo posteriormente.

## 29. Ley aplicable y jurisdicción

Estos Términos se rigen por la legislación española y por la normativa de la Unión Europea que resulte aplicable.

Cualquier controversia se resolverá ante los juzgados y tribunales que sean competentes conforme a la legislación aplicable.

Cuando la persona usuaria tenga la condición de consumidora, se respetarán en todo caso los derechos, fueros y mecanismos de protección que le reconozca la normativa de consumidores, incluido el tribunal correspondiente a su domicilio cuando legalmente proceda.

## 30. Identificación del prestador

Flare es un servicio prestado por:

- **Nombre comercial:** We Are Capa
- **Titular:** Miguel Varona Gallego
- **NIF:** 47024743R
- **Domicilio:** Calle Comandante Fontanes 81, Madrid, España
- **Correo electrónico de contacto:** [info@wearecapa.es](mailto:info@wearecapa.es)

En estos Términos, utilizamos las expresiones **"Flare"**, **"nosotros"** o **"el servicio"** para referirnos al prestador y a la aplicación.

## 31. Contacto

Para consultas sobre estos Términos, incidencias, reportes o solicitudes relacionadas con Flare, puedes escribir a:

**We Are Capa**
Titular: Miguel Varona Gallego
Calle Comandante Fontanes 81
Madrid, España
[info@wearecapa.es](mailto:info@wearecapa.es)`;
