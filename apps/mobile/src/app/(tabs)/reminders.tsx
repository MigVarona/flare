...
export default function RemindersScreen() {
  ...
  const addReminder = async () => {
    ...
    try {
      ...
      notice("No se ha podido guardar el aviso");
    } finally {
      setIsSending(false);
    }
  };
  ...
  const markDone = (reminder: Reminder) => {
    ...
    if (reminder.repeat && reminder.dueAt) {
      ...
      void updateDoc(doc(db, "spaces", spaceId, "reminders", reminder.id), {
        ...
        .then(announceDone)
        .catch(() => notice("No se ha podido completar el aviso"));
    ...
    const timeout = setTimeout(() => {
      ...
      void deleteDoc(doc(db, "spaces", spaceId, "reminders", reminder.id))
        ...
        .catch(() => {
          ...
          notice("No se ha podido completar el aviso");
        });
    }, UndoWindowMs);
  ...
  const postpone = async (reminder: Reminder, date: Date | undefined) => {
    ...
    try {
      ...
      notice("No se ha podido posponer el aviso");
    }
  };
  ...
  const removeConfirmed = async (reminder: Reminder) => {
    ...
    try {
      ...
      notice("No se ha podido borrar el aviso");
    }
  };
